...
const candidate = new URL('../../dist/sfhs-doom-android-sfhs-controls-v10.html', import.meta.url);
const evidenceRoot = resolve('..', 'test-results', 'P06', 'P6-058', 'screenshots');
...
async function installLifecycleProbe(page, fullscreenMode = 'success') {
  await page.addInitScript(mode => {
    ...
    Object.defineProperty(Element.prototype, 'requestFullscreen', { configurable: true, value: function requestFullscreen() {
      ...
      if (mode === 'reject') return Promise.reject(new Error('V10_TEST_FULLSCREEN_REJECTED'));
...
async function open(page, viewport, fullscreenMode = 'success') {
  test.skip(!existsSync(candidate), 'V10 artifact has not been built.');
...
for (const viewport of [
  ...
  test(`V10 ${viewport.name} portrait is edge-to-edge before startup`, async ({ page }) => {
    ...
    expect(game.height).toBeCloseTo(viewport.width * 5 / 8, 0);
...
test('V10 trusted fullscreen success starts immediately and drives native HUD through real controls', async ({ page }) => {
  ...
  expect(worldEdges.width).toBe(400); expect(worldEdges.height).toBe(250);
  ...
  expect(settingsBox.y).toBeGreaterThanOrEqual(250);
  ...
  expect((await page.evaluate(() => window.SFHS_WASM_TEST.mobileControlsProfile())).settings.opacity).toBe(beforeProfile.settings.opacity);
  await page.getByRole('button', { name: 'Open settings' }).click(); await page.getByRole('button', { name: 'Edit controls' }).click();
  await page.locator('#control-opacity').fill('0.47'); await page.getByRole('button', { name: 'Save layout' }).click();
  expect((await page.evaluate(() => window.SFHS_WASM_TEST.mobileControlsProfile())).settings.opacity).toBeCloseTo(.47);
  const downloadPromise = page.waitForEvent('download');
  await page.getByRole('button', { name: 'Open settings' }).click(); await page.getByRole('button', { name: 'Edit controls' }).click();
  await page.getByRole('button', { name: 'Export JSON' }).click(); await downloadPromise;
  await page.getByRole('button', { name: 'Reset' }).click();
  await page.evaluate(value => window.SFHS_WASM_TEST.setMobileControlsProfile(value), beforeProfile);
...
test('V10 fullscreen rejection is handled and Doom still starts exactly once', async ({ page }) => {
  const hygiene = watchPage(page);
  ...
  expect(value.fs.error).toContain('V10_TEST_FULLSCREEN_REJECTED');
...
test('V10 starts without a Fullscreen API', async ({ page }) => {
  const hygiene = watchPage(page);
...
test('V10 compatibility renderer boots with the detached native HUD', async ({ page }) => {
  const hygiene = watchPage(page);
  await open(page, { width: 400, height: 844 }, 'success');
