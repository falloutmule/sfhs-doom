...
class P6MobileContractTests(unittest.TestCase):
    ...
    def test_v15_centers_landscape_and_separates_editor_from_controls(self):
        ...
        self.assertIn("#minimap-region { position:absolute; z-index:2; left:env(safe-area-inset-left)", SHELL)
        self.assertIn("#control-deck { position:absolute; z-index:1; right:env(safe-area-inset-right)", SHELL)
        self.assertIn("#doom-status-region { position:absolute; z-index:3; left:env(safe-area-inset-left); right:env(safe-area-inset-right)", SHELL)
        self.assertIn("canvasCenterOffsetX", SHELL)
